<template>
  <div v-if="books" class="flex flex-col h-[88dvh]">
    <section
      class="flex items-end md:items-center justify-between sm:justify-start w-full px-4 pb-2 gap-y-2 gap-x-4 bg-black"
    >
      ...
      <div class="flex items-center min-w-28 gap-4">
        <label
          for="sortby"
          class="text-zinc-300 text-sm hidden sm:block md:text-base w-full text-left"
          >Sort by</label
        >
        <DropDown v-model="sort" :options="sortOptions" aria-label="Sort Books By" />
      </div>
    </section>
